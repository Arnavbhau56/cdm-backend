...
def run_auto_answer(deck) -> dict:
    """Core logic: runs GPT to find answers from all available context. Returns {founder_questions, updated}."""
    ...
    # Include names of uploaded materials as context signal
    material_names = list(deck.materials.values_list('name', flat=True))
    materials_context = ('\n'.join(f'- {n}' for n in material_names)) if material_names else 'No additional materials.'
